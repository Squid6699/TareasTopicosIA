...
def funcion_objetivo(*pos):
    """
    Evalúa la eficiencia de la colocación de sensores de humedad
    considerando topografía, cultivo y suelo.
    """
    ...
    for (x, y) in coords:
        # Simulación: elevaciones ideales entre 30–50 m
        elevacion = np.exp(-((x-50)**2 + (y-50)**2) / 1500)
        eficiencia_topo += elevacion

        # Simulación: zonas favorables para cultivos de maíz y tomate
        cultivo_factor = np.sin(x/10) * np.cos(y/10)
        eficiencia_cultivo += cultivo_factor

        # Simulación: suelos fértiles y bien drenados
        suelo_factor = np.exp(-abs(np.sin(x/15)))
        eficiencia_suelo += suelo_factor

    # Combinación ponderada de los tres factores
    eficiencia_total = (
        0.4 * eficiencia_topo +
        0.3 * eficiencia_cultivo +
        0.3 * eficiencia_suelo
    )

    # Como PSO minimiza, devolvemos el negativo para maximizar eficiencia
    return -eficiencia_total

# Número de sensores a colocar
n_sensores = 5

enjambre = Enjambre(
    n_particulas = 30,
    n_variables  = 2 * n_sensores,        # (x, y) por cada sensor
...
enjambre.optimizar(
    funcion_objetivo = funcion_objetivo,
    optimizacion     = "minimizar",       # porque devolvemos -eficiencia
    n_iteraciones    = 150,
    reduc_inercia    = True,
    parada_temprana  = True,
    rondas_parada    = 10,
...
mejor = enjambre.mejor_posicion
coords = np.array(mejor).reshape(-1, 2)
...
plt.ylabel("Y (m)")
plt.legend()
plt.title("Distribución óptima de sensores - Guasave, Sinaloa")
plt.show()
